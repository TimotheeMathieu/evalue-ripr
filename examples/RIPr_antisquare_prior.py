"""
In this example, we use the evalue_ripr package to project, in 2D, a mixture of Bernoulli distributions.
"""
...
prior = {"alpha":np.array([2,5]), "beta":np.array([1,1])}

prior_mean = prior["alpha"]/(prior["alpha"]+prior["beta"])
print("Mean of prior that we project is ", prior_mean)

# p, kinf = RIP_FW_multi([mycons1, mycons2], inits, 
#                         prior,use_posterior=True)

p, kinf = RIP_FW_multi([mycons1, mycons2], inits, 
                        {"prior_support":[prior_mean], "prior_weights":[1]}, # We project a mixture proba with only one point in the support
                        use_posterior=False)
...
# Plot the prior_mean and projection
ax.scatter([prior_mean[0]],[prior_mean[1]], color="blue", label="mean of prior to project", s = 50)
